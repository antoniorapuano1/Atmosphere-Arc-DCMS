                ...
                <div class="columns">
                  <div class="column">
                    ...
                    <div v-for="file in contract.data.attachments" v-bind:key="file.hash" style="border:1px solid #ccc; position:relative; text-align:left; color:#000; border-radius:5px; margin-top:20px; font-size:12px; padding:15px">
                        ...
                        <a v-bind:href="'https://ipfs.io/ipfs/'+file.hash"></a>
  ...
  export default {
    ...
    async mounted() {
      ...
      if(localStorage.blockchain==="Scrypta"){
        ...
        app.isArchived = isArchived*/
        let transactions=await app.algorand.transactionByAddress(app.$route.params.contract)
        ...
        for(let i=0;i<transactions.transactions.length; i++){
          if(transactions.transactions[i].from==transactions.transactions[i].payment.to){
             //contract.data=app.cipher.decrypt(body,localStorage.password)
             //let contract=await app.tempContract
             let contract={data: {title: ""}}
             contract.data=await app.getContract(transactions.transactions[i].note)
             console.log(contract)
              ...
              if(contract.data.contract !== undefined){
                ...
                for(let j in contract.data.subjects){
                  ...
                  app.identities[contract.data.subjects[j].address] = contract.data.subjects[j].address
                  let checkinteractions={data:{}}
                  checkinteractions.data=await app.algorand.transactionByAddress(contract.data.subjects[j].address)
                  console.log(signs)
                  
                  for(let jjj in checkinteractions.data.transactions){
                    let interaction = checkinteractions.data.transactions[jjj]
                    let note;
                    if(interaction.note!=undefined)
                      note=await app.getContract(interaction.note)
                      
                     if(note){
                       
                      if(interaction.sign !== undefined){
                        let expsign = interaction.refID.split(':')
                        let data = JSON.parse(interaction.data.message)
                        interaction.date = new Date(data.timestamp)
                        if(expsign.length === 3 && expsign[1] === 'general' && expsign[2] === contract.address){
                      ...
                      app.signs = signs
                      app.$forceUpdate();
                      if(interaction.notify === 'NOTIFY:' + app.$route.params.contract){
                        let data = JSON.parse(interaction.data.message)
                        interaction.data.message = data
                        if(data.attachments > 0 || data.plaintext !== ''){
                          if(data.plaintext !== ''){
    ...
    methods: {
      getContract(note){
        return new Promise(async resp => {
            if(algosdk.decodeObj(note)!="Send fee"){
              request(' https://ipfs.io/ipfs/'+algosdk.decodeObj(note), function (error, response, body) {
                  //let contract= {data: {title: ""}}
