            ...
            app.$buefy.dialog.prompt({
              ...
              onConfirm: async password => {
                if (password === localStorage.password) {
                  app.isBroadcasting = true
                  setInterval(async function(){
                    app.wallet=localStorage.algoWallet
                    app.scrypta.broadcast(JSON.parse(app.wallet.addr), password, 'message', JSON.stringify({action: 'join', name: app.name, surname: app.surname, contract: app.$route.params.address, time: new Date()}))
                  },3000)
                } else {
                  app.$buefy.toast.open({
                    message: "Wrong password!",
